function Assign() {
    ...
    */
    ...
    console.log(`${model1},${color1},${year1}`);

    //ES6 분해할당
    const {color, model, year} = car; //키 값으로 가져오는 것이기에 순서는 상관 X
    console.log(`model = ${model}, color = ${color}, year = ${year}`);

    const {model:m2, color:c2, year: ye2} = car; // 변수명 대체를 원한다면
    console.log(`model2 = ${m2}, color2 = ${c2}, year2 = ${ye2}`);
