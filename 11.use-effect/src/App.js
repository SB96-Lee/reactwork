...
function App() {
  ...
  */
  ...
  // 모든 업데이트가 실행될 때(update) => 배열은 넣지 않음
  useEffect(() => {
    console.log("update");
  });

  // 배열에 [count] => count 값이 update 될 때만  호출
  useEffect(() => {
    console.log("count update");
  },[count]);

  // count와 text의 값이 update 될 때만 실행
  useEffect(() => {
